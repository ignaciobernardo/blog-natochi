...
export const MAX_MEDIA_SIZE = 30 * 1024 * 1024; // 30MB for media (photos/videos)
...
// Media (photos/videos) validation schema - 30MB limit
export const mediaBlobSchema = z
  .instanceof(Blob)
  .refine((file) => file.size <= MAX_MEDIA_SIZE, {
    message: 'File size must be less than 30MB',
  })
  .refine((file) => ALLOWED_MEDIA_TYPES.includes(file.type), {
    message:
      'File must be an image (JPEG, PNG, WebP) or video (MP4, MOV, WebM)',
  });
