...
interface SendSelfFinanceExpiredEmail25Params {
  submissionId: string;
}

export class SelfFinanceExpiredEmailSender25 {
  async sendToAllMembers(
    params: SendSelfFinanceExpiredEmail25Params,
  ): Promise<void> {
    try {
      ...
      await Promise.all(
        submissionDetails.members.map((member) => {
          ...
          return sendEmail({
            ...
            to: member.email,
            subject: `Platanus Hack 25 - Plazo vencido - ${githubUsername}`,
            sentByUserId: null,
          });
        }),
      );
...
export const selfFinanceExpiredEmailSender25 =
  new SelfFinanceExpiredEmailSender25();
