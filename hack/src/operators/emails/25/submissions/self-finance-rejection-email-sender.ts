...
interface SendSelfFinanceRejectionEmail25Params {
  submissionId: string;
  sentByUserId: string | null;
}

export class SelfFinanceRejectionEmailSender25 {
  async sendToAllMembers(
    params: SendSelfFinanceRejectionEmail25Params,
  ): Promise<void> {
    try {
      ...
      await Promise.all(
        submissionDetails.members.map((member) => {
          ...
          return sendEmail({
            ...
            to: member.email,
            subject: `Platanus Hack 25 - ${githubUsername}`,
...
export const selfFinanceRejectionEmailSender25 =
  new SelfFinanceRejectionEmailSender25();
