...
import VotingStartedEmail from '@/src/emails/projects/voting-started';
...
export class VotingStartedSender {
  async sendToAllTeamMembers(): Promise<void> {
    try {
      ...
      console.log(
        `Found ${projectsWithMembers.length} projects to send voting started emails for`,
      );
      ...
      for (const project of projectsWithMembers) {
        const emailPromises = project.members.map(async (member) => {
          ...
          const subject = `La votación pública ya empezó | ${project.projectName}`;
          ...
          await sendEmail({
            templateName: 'voting-started',
            template: VotingStartedEmail,
            templateProps: {
          ...
          console.log(
            `✅ Queued voting started email for ${member.fullName} (${member.email}) - Project: ${project.projectName}`,
          );
        });
      ...
      console.log(
        `\n🎉 Successfully queued ${totalEmailsSent} voting started emails for ${projectsWithMembers.length} projects`,
      );
    } catch (error) {
      console.error('Failed to send voting started emails:', error);
      throw error;
    }
  }
...
export const votingStartedSender = new VotingStartedSender();
