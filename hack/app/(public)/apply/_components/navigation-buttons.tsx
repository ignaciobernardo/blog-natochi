...
export function NavigationButtons({
  ...
  const isStepComplete = (): boolean => {
    ...
    if (currentStep.includes('member-personal')) {
      ...
      return !!(
        ...
        validateGitHubUrl(member.githubProfile) &&
        validateLinkedInUrl(member.linkedinProfile)
      );
    }
  ...
  const validateCurrentStep = (): boolean => {
    ...
    if (currentStep === 'modality' && !formData.modality) {
    ...
    if (currentStep.includes('member-personal')) {
      ...
      if (!validateGitHubUrl(member.githubProfile)) {
        setError('GitHub URL must be in format: https://github.com/username');
        return false;
      }

      if (!validateLinkedInUrl(member.linkedinProfile)) {
        setError(
  ...
  const isFirstStep = currentStep === 'welcome-1';
