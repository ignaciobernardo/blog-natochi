...
export function ProgressBar({ currentStep }: ProgressBarProps) {
  ...
  const steps = useStepSequence().filter((step) => step !== 'modality');
  const normalizedCurrentStep =
    currentStep === 'modality' ? 'team-status' : currentStep;
  const normalizedMaxStepReached =
    maxStepReached === 'modality' ? 'welcome-2' : maxStepReached;

  const maxIndex = steps.indexOf(normalizedMaxStepReached);
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="font-semibold text-lg">
        {STEP_LABELS[normalizedCurrentStep] || normalizedCurrentStep}
      </div>
    </div>
  );
}
