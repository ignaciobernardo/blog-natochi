...
const Description = () => {
  ...
  const paragraphs = [
    [
      ...
      {
        text: ' de cero a producto en 36 horas.',
  ...
  useEffect(() => {
    const splitTextIntoLines = () => {
      ...
      tempElement.style.font = window.getComputedStyle(
      ...
      const processText = (paragraph) => {
        ...
        words.forEach((wordObj) => {
          const testLine =
            currentLine + (currentLine ? ' ' : '') + wordObj.word;
          tempElement.textContent = testLine;
          const lineWidth = tempElement.offsetWidth;

          if (lineWidth > containerWidth - 50) {
            // 50px buffer for line number
            if (currentLine) {
              newLines.push({
                number: lineNumber++,
                content: currentLineSegments,
              });
              currentLine = wordObj.word;
              currentLineSegments = [
                { text: wordObj.word, style: wordObj.style },
              ];
            } else {
              currentLine = wordObj.word;
              currentLineSegments = [
            ...
            if (
              ...
              currentLineSegments[currentLineSegments.length - 1].style ===
                wordObj.style
            ) {
  ...
  const renderLine = (line) => {
    return line.content.map((segment) => (
      <span
        key={`${line.number}-${segment.style ?? 'plain'}-${segment.text}`}
      ...
      <Padding>
        <div className="flex flex-col md:flex-row">
          <div className="flex flex-col md:w-1/2">
            <TypewriterTitle
              text="$ vim hack-description.txt"
              className="mb-12 text-center font-bold font-oxanium text-2xl md:text-5xl"
            />
            <div
              ref={containerRef}
              className="mx-auto mb-16 w-full max-w-2xl whitespace-pre-wrap text-left font-mono leading-relaxed"
            >
              {lines.map((line) => (
                <div key={line.number} className="flex">
                  <span className="mr-4 w-8 flex-shrink-0 text-right text-zinc-500">
                    {line.number}
                  </span>
          ...
          <div className="flex items-center justify-center md:grow">
            <div
              ref={tilesRef}
              className="grid grid-cols-2 grid-rows-2 items-center justify-center gap-0"
            >
              {logoTiles.map((tile, index) => (
                ...
                >
                  <Image
                    ...
                    width={150}
                    height={150}
                    className={`m-0 box-border p-0 opacity-0 transition-opacity duration-500 ease-in-out ${isVisible ? 'opacity-100' : ''}
                    `}
                    style={{
