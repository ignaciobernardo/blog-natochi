(() => {
 ...
 const paper=document.querySelector('#paper'),drawing=document.querySelector('#drawing'),corners=document.querySelector('#corners');
 ...
 function renderCorners(){
  const keepouts=[...document.querySelectorAll('.masthead h1,.masthead span,.versions,.actions')].map(e=>{const r=e.getBoundingClientRect();return{x:r.x,y:r.y,width:r.width,height:r.height};}).filter(r=>r.width&&r.height);
  PrintedCorners.render(corners,{wear,keepouts});
 }
 function render(){
  const dpr=Math.min(devicePixelRatio||1,1.6),w=Math.round(innerWidth*dpr),h=Math.round(innerHeight*dpr);
  paper.width=drawing.width=corners.width=w;paper.height=drawing.height=corners.height=h;
  Paper.render(paper,paperId);PulsarMap.render(drawing,{mode,wear});renderCorners();
 ...
 document.querySelector('#wear').oninput=e=>{wear=Number(e.target.value);PulsarMap.render(drawing,{mode,wear});renderCorners();};
 ...
 document.querySelector('#export').onclick=()=>{const out=document.createElement('canvas');out.width=paper.width;out.height=paper.height;const c=out.getContext('2d');c.drawImage(paper,0,0);c.globalCompositeOperation='multiply';c.drawImage(corners,0,0);c.drawImage(drawing,0,0);out.toBlob(blob=>{const url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=`pulsar-${mode}-paper-${paperId}.png`;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);});};
