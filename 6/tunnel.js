...
(() => {
 ...
 const trails=Array.from({length:68},()=>({angle:random()*Math.PI*2,start:.2+random()*.46,length:.13+random()*.5,wave:random()*Math.PI*2,amplitude:.015+random()*.035,dashed:random()<.67,brightness:.5+random()*.4}));
 function position(t,angle){
  const r=inner*Math.exp(t*span);
  const throat=Math.exp(-(((t-.36)/.21)**2));
  const neck=Math.exp(-(((t-.20)/.15)**2));
  const a=angle;
  // A single smooth bend near the throat; outer rays keep their direction.
  const bendX=r*.32*throat;
  const bendY=-r*.42*throat;
  const asymmetry=1;
 ...
 function draw(){
  ...
  // Regular elliptical contours and directional meridians form the funnel.
  ctx.strokeStyle='rgba(179,172,161,0.82)';ctx.lineWidth=.95;
  for(let ring=0;ring<29;ring++){
   const t=ring/28;
   const points=[];
   for(let j=0;j<=210;j++)points.push(position(t,j/210*Math.PI*2));
   path(points);
  }
  ctx.strokeStyle='rgba(192,184,172,0.75)';ctx.lineWidth=1;
  for(let spoke=0;spoke<17;spoke++){
   const points=[];
   for(let j=0;j<=190;j++){const t=j/190;points.push(position(t,spoke/17*Math.PI*2));}
   path(points);
  }
  ...
  for(const trail of trails){
   ...
   for(let j=0;j<=125;j++){
    ...
    const angle=trail.angle+trail.amplitude*Math.sin((t-trail.start)/trail.length*Math.PI+trail.wave);
    points.push(position(t,angle+time*.00065));
   }
   path(points);
  }
  ctx.setLineDash([]);
  for(const star of stars){
   const p=position(star.depth,star.angle+time*.00065);
